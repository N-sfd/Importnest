    ...
    <div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((product) => {
          const stale = isFreshnessStale(product.lastCheckedMinutesAgo);
          return (
            <article key={product.id} className="panel flex h-full flex-col gap-2 p-4">
              ...
              <dl className="mt-1 space-y-1 text-xs">
                <div className="flex justify-between gap-2">
                  <dt className="text-muted">Lowest Total Known Cost</dt>
                  <dd className="font-semibold tabular-nums text-navy-900">
                    {product.lowestKnownPrice != null
                      ? `$${product.lowestKnownPrice.toFixed(2)}`
                      : "Not provided"}
                  </dd>
                </div>
                <div className="flex justify-between gap-2">
                  ...
                  <dd className="font-semibold text-navy-900">
                    {product.offerCount > 0 ? product.offerCount : "Not provided"}
                  </dd>
                </div>
                <div className="flex justify-between gap-2">
                  <dt className="text-muted">Sources</dt>
                  <dd className="font-semibold text-navy-900">
                    {product.sourceCount > 0 ? product.sourceCount : "Not provided"}
                  </dd>
                </div>
                <div className="flex justify-between gap-2">
              ...
              <div className="mt-auto flex gap-2 pt-2">
                <Link
                  ...
                  className="btn-cta flex-1 px-3 py-1.5 text-center text-xs"
                >
                  View offers
                </Link>
                <button
